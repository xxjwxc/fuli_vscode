...
export function registerViewEvent(
  ...
  commands.registerCommand('a-shares.yjzd', async () => {
      ...
      if (yjzd) {
        ...
        const res = await Axios.post('https://www.xxjwxc.cn/shares/api/v1/shares.yjzd_code',{codes: codes},
          {
            headers : {
  ...
  // 删除股票
   commands.registerCommand('a-shares.deleteStockOnline', (target) => {
    LeekFundConfig.removeStockCfg(target.id, () => {
      ...
      Axios.post('https://www.xxjwxc.cn/shares/api/v1/shares.delete_my_code',{		code: target.id},
        {
          headers : {
            'Content-Type': 'application/json',
            'user-token': userId
