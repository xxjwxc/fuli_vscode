...
export default class ChatViewProvider implements vscode.WebviewViewProvider {
  ...
  public async clearUser() {
    ...
    const res = await Axios.post('https://www.xxjwxc.cn/shares/api/v1/weixin.clear_login', { code: deviceId });
    console.log(res)
  ...
  public async updateUser() {
    const deviceId = globalState.deviceId;
    const res = await Axios.post('https://www.xxjwxc.cn/shares/api/v1/weixin.h5_login', { code: deviceId });
    if (res.status === 200) {
      const data = res.data;
      if (data.state === true) {
  ...
  private _getHtmlForWebview() {
    ...
    if (userid.length > 0) {// 已经登陆了
      ...
      const url = `https://www.xxjwxc.cn/pc/#/chat?username=${userid}`;
      return `<!DOCTYPE html>
			<html lang="en">
			  <head>
				<meta charset="UTF-8" />
       ...
       const url = `https://www.xxjwxc.cn/webshares/#/pages/mine/login?deviceid=${globalState.deviceId}`;
      // const url = `https://www.xxjwxc.cn/pc/#/login?deviceid=${globalState.deviceId}`;
      return `<!DOCTYPE html>
			<html lang="en">
			  <head>
				<meta charset="UTF-8" />
