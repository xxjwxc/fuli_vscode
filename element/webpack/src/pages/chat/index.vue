...
const submit = () => {
    ...
    const eventSource = new EventSource(`https://www.xxjwxc.cn/shares/api/v1/analy.chat?msg=${encodeURIComponent(value.value)}&user-token=${userInfo.openid}`)
    // // 与事件源的连接刚打开时触发
...
const qCloud = () => {
    window.parent.postMessage({
        type: 'home',
        url: 'https://www.xxjwxc.cn'
    }, '*');
}


fetch('https://www.xxjwxc.cn/shares/api/v1/file/xlsx/codes.json').then(res => res.json()).then(data => { jsonData.value = data })
